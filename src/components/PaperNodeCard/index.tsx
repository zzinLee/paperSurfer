...
interface PaperNodeCardProps {
  nodeData: RootConfig;
  setModalOpen: Dispatch<SetStateAction<boolean>>;
  setIsLoadingChild: Dispatch<SetStateAction<boolean>>;
}
...
function PaperNodeCard({ nodeData, setModalOpen, setIsLoadingChild }: PaperNodeCardProps) {
  ...
  const abstract = paper && paper.abstract && decodedString(paper.abstract).replaceAll("Abstract", "").slice(0, 290);
  const formattedAbstract = abstract ? abstract + "..." : "초록 정보 없음";
  const isStar = nodeData.status === STATUS.STAR;

  const fetchPaperInformation = async (doi: string, setPaper: Dispatch<SetStateAction<null | PaperConfig>>) => {
  ...
  const getChildrenNode = async (collectionId: string, parentNode: RootConfig) => {
    ...
    const response = res?.data?.message;
    const paperRefList: ReferenceConfig[] = response?.items?.[0]?.reference;
    ...
    const formattedChildrenList = childrenList?.map((child: SearchResponseConfig): RootConfig => {
  ...
  const clickStar = () => {
    ...
    if (!isChildrenExist && isReferenceExist) {
      getChildrenNode(collectionId, nodeData);
    }
  ...
  const clickPaperLink = (ev: MouseEvent<HTMLAnchorElement>) => {
    ev.preventDefault();
        ...
        <div className="flex flex-col bg-white border rounded-lg shadow border-slate-700 max-w-[900px]">
          ...
          <div className="px-12 py-8">
            <h1 className="px-12 py-4 m-2 font-semibold tracking-tight">
              <p className="p-4">{paper && decodedString(paper?.title) || "제목 정보 없음"}</p>
            </h1>
            <div className={CLASS_CARD_PROP}>
              <p className={`${CLASS_BADGE} bg-slate-100 text-slate-600`}>저널명</p>
              <p className="text-sm text-slate-600">{paper && decodedString(paper?.containerTitle) || "저널 정보 없음"}</p>
            </div>
            <div className={CLASS_CARD_PROP}>
            ...
            <div className="inline-flex">
              <div className={CLASS_CARD_PROP}>
                ...
                <p className="text-sm text-slate-600 max-w-680">{paper && decodedString(paper?.author) || "저자 정보 없음"}</p>
              </div>
            </div>
            <div className={CLASS_CARD_PROP}>
